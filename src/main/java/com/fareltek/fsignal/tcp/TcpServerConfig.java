    ...
    public DisposableServer tcpDisposableServer(TcpConnectionHandler handler) {
        DisposableServer server = TcpServer.create()
                ...
                .doOnConnection(connection -> {
                    String remoteAddr = connection.channel().remoteAddress().toString();
                    log.info("[TCP] Yeni baglanti: {}", remoteAddr);

                    connection.inbound().receive()
                            .asByteArray()
                            .doOnNext(bytes -> handler.onData(remoteAddr, bytes))
                            .doOnComplete(() -> log.info("[TCP] Baglanti kapandi: {}", remoteAddr))
        ...
        log.info("  Waveshare RS485/ETH baglantisi bekleniyor...");
        log.info("========================================");
